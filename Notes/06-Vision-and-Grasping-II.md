...
ICP 用于优化初始位姿估计，通过迭代优化使传感器点云 $Q = \{q_1, q_2, \cdots, q_m\}$ 与模型点云 $P = \{p_1, p_2, \cdots, p_n\}$ 对齐。设物体位姿为 $(R, t)$，能量函数（Energy）定义为：

$$
E(R,t) = \frac{1}{|P|} \sum_{i=1}^{|P|} \| (R p_i + t) - q_j \|^2
$$

其中 $q_j$ 是 $p_i$ 在 $Q$ 中的最近邻点。算法分三步迭代：

1. **数据预处理** （Data Centering）
    - 计算均值 $\bar{P} = \frac{1}{n}\sum p_i$, $\bar{Q} = \frac{1}{m}\sum q_j$
    - 中心化点云：$\hat{p}_i = p_i - \bar{P}$, $\hat{q}_j = q_j - \bar{Q}$
    - 这一步的目的是先去除位移 $t$ 的影响
...
4. **迭代优化** （Iteration）
    - 将求得的 $(R, t)$ 应用到未居中的点云 $P$，得到新的点云 $P'$，重复步骤 1~3

迭代终止条件通常设置为 $\Delta R, \Delta t < \epsilon$ 或 $\Delta E < \epsilon$，或达到最大迭代次数（如 30 次）。
...
Instance level 的位姿估计都要求我们知道物体的完整建模，否则我们缺乏目标。

不过，对于一些有自然定义的 pose，它会具有一个天然的参考性（从而提供一个类别级的参考系），从而可以从 Instance level 延拓到 Category level，直接对这一类别的物体的 pose 进行预测。这是王鹤老师在 CVPR 2019 Oral 的工作，原始论文可以参见 [这里](https://arxiv.org/abs/1901.02970)。

当物体缺乏 CAD 模型时，需建立类别级的统一参考系。
...
3. **尺寸归一化** （Scale Normalization）：通过对角线长度限制 Bounding box 的大小（对角线长度为 $1$，一定能装到 $1\times 1\times 1$ 的 Bounding box 内）
...
刚才介绍过了 NOCS 方法，那么现在最大的问题就在于如何去训练这样一个从二维 RGB 图像重建到 NCOS 空间的神经网络了。
...
> 王老师还提到，目前 Pose Estimation 领域最 work 的模型就是纯合成数据训练出来的，不过他们的合成过程会更加精细
>
...
定义：抓取是指通过在接触点施加力和力矩，以期望的方式约束物体运动的过程。
...
传统的抓取问题可以看作是一个回归问题，即预测唯一的抓取位姿。然而，由于遮挡和对称性，一个物体通常存在多个可行的抓取。因此，将抓取建模为一个生成问题更为合适。
