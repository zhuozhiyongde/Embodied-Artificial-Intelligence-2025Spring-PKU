...
轴角表示由一个单位向量 $\mathbf{e} = [e_x, e_y, e_z]^T$（表示旋转轴）和一个标量 $\theta$（表示旋转角度）组成：
...
2. 球大圆上的不连续性：由于双重覆盖，我们只能取一个半圆，但是在这个切的直径上，我们还是只能选取两个切点中的一个（否则又存在双重覆盖问题，$q = -q$），而这么选取的话，在这个点附近，依旧有类似欧拉角的跳变存在（还是那个原因，在这个点附近的微小变动会引发跳变）
...
这种表示实际上只有 6 个自由度，所以我们叫他 6D 表示方法。
...
这种方法建立了两个坐标系之间的对应关系：模型坐标系 $(x_i^M, y_i^M, z_i^M)$ 和相机坐标系 $(x_i^C, y_i^C, z_i^C)$。
...
不考虑位移 $t$ 的纯旋转拟合可以形式化为正交 Procrustes 问题，这是一个矩阵逼近问题。给定矩阵 $\mathbf{M} \in \mathbb{R}^{n \times p}$ 和 $\mathbf{N} \in \mathbb{R}^{n \times p}$，我们需要求解：

$$
\hat{\mathbf{A}} = \arg\min_{\mathbf{A} \in \mathbb{R}^{p \times p}} \|\mathbf{M} - \mathbf{NA}\|_F^2 \quad \text{subject to} \quad \mathbf{A}^T\mathbf{A} = \mathbf{I}
$$

其中，$\|\cdot\|_F$ 表示 Frobenius 范数，定义为：

$$
\|X\|_F = \sqrt{\text{trace}(X^TX)} = \sqrt{\sum_{i,j} x_{ij}^2}
$$
...
-   求解的 $\mathbf{A}$ 即为从 $\mathbf{N}$ 到 $\mathbf{M}$ 的最优旋转矩阵
    ...
    约束条件 $\mathbf{A}^T\mathbf{A} = \mathbf{I}$ 确保 $\mathbf{A}$ 是正交矩阵，保证了纯旋转变换（不包含缩放或剪切）。

正交 Procrustes 问题有一个优雅的解析解，可以通过奇异值分解（SVD）获得。如果我们对矩阵 $\mathbf{M}^T\mathbf{N}$ 进行 SVD 分解：

$$
\mathbf{M}^T\mathbf{N} = \mathbf{UDV}^T
$$

那么最优旋转矩阵为：

$$
\hat{\mathbf{A}} = \mathbf{VU}^T
$$
...
3. 转置性质：$\text{tr}(A^T) = \text{tr}(A)$
4. 标量提取：$\text{tr}(cA) = c·\text{tr}(A)$，其中 $c$ 为标量
5. 与 Frobenius 范数的关系：$\|A\|_F^2 = \text{tr}(A^TA) = \text{tr}(AA^T)$

利用迹运算的性质和 $\mathbf{A}$ 是正交矩阵的条件（$\mathbf{A}^T\mathbf{A} = \mathbf{I}$）：

$$
\begin{aligned}
\|\mathbf{M} - \mathbf{NA}^T\|_F^2
&= \text{tr}((\mathbf{M} - \mathbf{NA}^T)^T(\mathbf{M} - \mathbf{NA}^T))\\
&= \text{tr}(\mathbf{M}^T\mathbf{M}) - 2\text{tr}(\mathbf{M}^T\mathbf{NA}^T) + \text{tr}(\mathbf{AN}^T\mathbf{NA}^T) \\
&= \text{tr}(\mathbf{M}^T\mathbf{M}) - 2\text{tr}(\mathbf{M}^T\mathbf{NA}^T) + \text{tr}(\mathbf{N}^T\mathbf{N})
\end{aligned}
$$

注意到第一项 $\text{tr}(\mathbf{M}^T\mathbf{M})$ 和第三项 $\text{tr}(\mathbf{N}^T\mathbf{N})$ 都不依赖于 $\mathbf{A}$，因此最小化目标函数等价于最大化第二项 $\text{tr}(\mathbf{M}^T\mathbf{NA}^T)$。

当我们有 SVD 分解 $\mathbf{M}^T\mathbf{N} = \mathbf{UDV}^T$ 时，可以将迹运算展开：

$$
\begin{aligned}
\text{tr}(\mathbf{M}^T\mathbf{NA}^T) &= \text{tr}(\mathbf{UDV}^T\mathbf{A}^T) \\
&= \text{tr}(\mathbf{UD}(\mathbf{AV})^T) \\
&= \text{tr}((\mathbf{AV})^T\mathbf{UD}) \\
&= \sum_{i=1}^{d}[(\mathbf{AV})^T\mathbf{U}]_{ii}d_i
\end{aligned}
$$

其中 $d_i$ 是矩阵 $\mathbf{D}$ 对角线上的第 $i$ 个元素，$d$ 是 $\mathbf{M}^T\mathbf{N}$ 的非零奇异值的数量。

为了最大化上述和式，我们需要使 $(\mathbf{AV})^T\mathbf{U}$ 的对角元素尽可能大。由于 $\mathbf{AV}$ 和 $\mathbf{U}$ 都是正交矩阵，因此 $(\mathbf{AV})^T\mathbf{U}$ 也是正交矩阵，其对角元素的绝对值不能超过 1（否则对应的列 / 行的 $L_2$ 范数会超过 1）。

因此，该和式在所有 $(\mathbf{AV})^T\mathbf{U}$ 的对角元素都等于 1 时达到最大值，即：

$$
\begin{aligned}
(\mathbf{AV})^T\mathbf{U} &= \mathbf{I} \\
\mathbf{AV} &= \mathbf{U} \\
...
**最小二乘法** （Least Square Method）：假设有一组数据点 $(x_i, y_i)$，我们希望通过直线 $y = mx + b$ 拟合这些点。
...
对物体级别的位姿变换预测，要求每个物体都已知（完整建模），典型算法如 PoseCNN，如果结合 ICP 算法可以在位移幅度较小的情况下更快的提升准确率。
