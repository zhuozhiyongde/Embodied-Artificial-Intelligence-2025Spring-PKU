...
class Obs:
    ...
    seg: np.ndarray
    """(H, W) segmentation mask, each pixel is the object id, 0 for background"""
...
class WrapperEnv:
    ...
    def launch(self):
        ...
        if self.config.table_pose is not None and self.config.table_size is not None:
            ...
            self.config.table_pose = to_pose(
                np.array(
                    [
                        np.random.uniform(0.55, 0.6),  # x: slightly expanded range
                        np.random.uniform(0.38, 0.47),  # y: table center position
                        np.random.uniform(0.68, 0.74),  # z: table height
                    ]
                )
            )
            
            self.config.table_size = np.array(
                [
                    np.random.uniform(0.68, 0.72),  # table width
                    np.random.uniform(0.36, 0.42),  # table depth
                    0.02,  # table thickness (fixed)
                ]
            )
            table_pose = self.config.table_pose
            table_size = self.config.table_size
    ...
    def get_obs(self, camera_id: int = 1) -> Obs:
        ...
        # 简洁方案：直接使用原始分割图
        seg = x["seg"][..., 0].astype(np.uint8)  # (H, W) 原始分割掩码
        
        obs = Obs(
            rgb=x["rgb"],
            depth=x["depth"],
            seg=seg,  # 包含所有物体ID的原始分割图
